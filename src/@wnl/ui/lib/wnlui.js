...
var picker$1 = function () {
  function picker() {
    ...
    this.defaultIndexList = cacheConfirmObj[this.defaults.id] ? util.util.deepCopy(cacheConfirmObj[this.defaults.id]) : this.defaults.defaultIndex || [];
    ...
    this.changeItemList = util.util.deepCopy(this.itemList);
    ...
    this.$picker.on('click', '.wnlui_mask', function () {
      ...
      cacheConfirmObj[_this.defaults.id] = util.util.deepCopy(_this.defaultIndexList);
  ...
  picker.prototype.setResultList = function setResultList() {
    ...
    this.changeItemList.forEach(function (item, i) {
      ...
      _this3.resultList[i] = util.util.deepCopy(item[_this3.defaultIndexList[i]]);
...
var datePicker = function () {
  ...
  datePicker.prototype.getChangeDateObj = function getChangeDateObj() {
    ...
    if (confirmObj.isSolar) {
      ...
      confirmObj.dateObj = util.calendar.solar2lunar(year, month, day);
    } else {
      var _year = this.defaultIndexList[0] + this.minLDate[0];
      var _month = this.defaultIndexList[1];
      var _day = this.defaultIndexList[2] + 1;
      var solarObj = void 0;
      if (util.calendar.leapMonth(_year) > 0) {
        if (_month < util.calendar.leapMonth(_year)) {
          _month += 1;
          solarObj = util.calendar.lunar2solar(_year, _month, _day);
        } else if (_month === util.calendar.leapMonth(_year)) {
          solarObj = util.calendar.lunar2solar(_year, _month, _day, true);
        } else {
          solarObj = util.calendar.lunar2solar(_year, _month, _day);
        }
      } else {
  ...
  datePicker.prototype.dateLunarTap = function dateLunarTap() {
    ...
    if (lunarType) {
      ...
      var month = this.defaultIndexList[1];
      var day = this.defaultIndexList[2] + 1;
      if (util.calendar.leapMonth(year) > 0) {
        if (month < util.calendar.leapMonth(year)) {
          month += 1;
        } else if (month === util.calendar.leapMonth(year)) {
          isLeap = true;
        }
      } else {
        month += 1;
      }
      var solarObj = util.calendar.lunar2solar(year, month, day, isLeap);
  ...
  datePicker.prototype.initSolarDateSelect = function initSolarDateSelect(year, month, day) {
    ...
    var dayCount = util.calendar.solarDays(year, month);
    ...
    for (i = 1; i <= dayCount; i += 1) {
      ...
      if (year === this.dateNow.getFullYear() && month === this.dateNow.getMonth() && i === this.dateNow.getDate()) {
        ...
        dayList.items.push({
          label: i + '日周' + util.calendar.nStr1[dateObj.getDay()]
        });
      }
    }
    if (day > dayCount) {
  ...
  datePicker.prototype.initLunarDateSelect = function initLunarDateSelect(year, month, day) {
    var i = 0;
    var lunarObj = util.calendar.solar2lunar(year, month, day);
    var yearList = {
      items: []
    };
    var yearToView = 0;
    ...
    var leapMonth = util.calendar.leapMonth(lunarObj.lYear);
    if (leapMonth > 0) {
      monthCount += 1;
    }
    for (i = 0; i <= monthCount; i += 1) {
      ...
      if (leapMonth > 0) {
        if (i < leapMonth) {
          monthList.items.push({
            label: util.calendar.toChinaMonth(i + 1)
          });
        } else if (i === leapMonth) {
          monthList.items.push({
            label: '闰' + util.calendar.toChinaMonth(leapMonth)
          });
        } else {
          monthList.items.push({
            label: util.calendar.toChinaMonth(i)
          });
        }
      } else {
        monthList.items.push({
    ...
    var dayCount = leapMonth === lunarObj.lMonth ? util.calendar.leapDays(lunarObj.lYear) : util.calendar.monthDays(lunarObj.lYear, lunarObj.lMonth);
    for (i = 1; i <= dayCount; i += 1) {
      if (lunarObj.lDay === i) {
        dayToView = i - 1;
      }
      solarObj = util.calendar.lunar2solar(lunarObj.lYear, lunarObj.lMonth, i, lunarObj.isLeap);
      if (solarObj.cYear === this.dateNow.getFullYear() && solarObj.cMonth - 1 === this.dateNow.getMonth() && solarObj.cDay === this.dateNow.getDate()) {
        ...
        dayList.items.push({
          label: util.calendar.toChinaDay(i) + '周' + util.calendar.nStr1[solarObj.nWeek]
        });
      }
    }
    this.dateGroups[0] = yearList;
  ...
  datePicker.prototype.getDateSelectMonthInfo = function getDateSelectMonthInfo(lastIndex) {
    ...
    if (!lunarType) {
      ...
      month = this.defaultIndexList[1];
      var leapMonth = util.calendar.leapMonth(year);
      var monthCount = 11;
      if (leapMonth > 0) {
        monthCount += 1;
      }
      for (i = 0; i <= monthCount; i += 1) {
        if (month === i) {
          ...
          if (leapMonth > 0) {
            ...
            //现在选择的是没有闰月的
            //判断之前的选择是否有闰月
            var prevLeapMonth = lastIndex ? util.calendar.leapMonth(lastIndex + +this.minLDate[0]) : 0;
  ...
  datePicker.prototype.getDateSelectDayInfo = function getDateSelectDayInfo() {
    ...
    if (!lunarType) {
      ...
      if (util.calendar.leapMonth(_year3) > 0) {
        if (_month3 < util.calendar.leapMonth(_year3)) {
          _month3 += 1;
        } else if (_month3 === util.calendar.leapMonth(_year3)) {
          isLeap = true;
        }
      } else {
        _month3 += 1;
      }
      var _dayCount = isLeap ? util.calendar.leapDays(_year3) : util.calendar.monthDays(_year3, _month3);
      var solarObj = {};
      for (i = 1; i <= _dayCount; i += 1) {
        if (_day3 === i) {
          dayToView = i - 1;
        }
        solarObj = util.calendar.lunar2solar(_year3, _month3, i, isLeap);
        dayList.items.push({
          index: i,
          label: util.calendar.toChinaDay(i) + '周' + util.calendar.nStr1[solarObj.nWeek]
        });
...
var cityPicker = function () {
  function cityPicker(options) {
    ...
    if (this.defaults.cityCode) {
      cityData = util.util.deepCopy(importCityCodeData);
      ...
      cityData = util.util.deepCopy(importCityData);
...
exports.pageloading = pageloader$1;
exports.wnlShare = wnlShare;
exports.wxShare = setShareParams;
